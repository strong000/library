...
@section('content')
    <div class="col-12 mt-5">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title">Edit Movie : {{ $movie->title }}</h4>
                    <form role="form" method="POST" action="{{ route('movie_update', $movie->id) }}">
                        {!! csrf_field() !!}
                        <div class="form-group">
                            <label for="title" class="col-form-label">Title</label>
                            <input id="title" class="form-control" type="text" value="{{ $movie->title }}" name="title" required>
                        </div>
                        <div class="form-group">
                            <label for="genre" class="col-form-label">Genre</label>
                            <input id="genre" class="form-control" type="text" value="{{ $movie->genre }}" name="genre" required>
                        </div>
                        <div class="form-group">
                            <label for="released_date" class="col-form-label">Released Date</label>
                            <input id="released_date" class="form-control" type="date" value="{{ $movie->released_date }}" name="released_date" required>
                        </div>
